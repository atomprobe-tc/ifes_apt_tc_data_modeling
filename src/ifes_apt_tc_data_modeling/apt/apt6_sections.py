...
class AptFileSectionMetadata:
    ...
    def get_ametek_count(self) -> np.uint64:
        ...
        return self.meta["ll_record_count"] * np.uint64(
            self.meta["i_record_size"] / (self.meta["i_data_type_size"] / 8)
        )
    ...
    def get_ametek_shape(self) -> list:
        ...
        return [
            ...
            np.uint64(
                np.uint64(self.meta["i_record_size"])
                / (self.meta["i_data_type_size"] / 8)
            ),
        ]
